...
Brekeke.Phonebook = {
	...
	_p : {
		join : function( ary, sep ){
			...
			for( var a = 0; a < ary.length; a++ ){
				...
				if( !t.substring ){  //assuming array or Java array
					t = join( t, sep );
				}
				t = t.trim();
				if( t.length == 0 ){
	...
	getManager_en : function(){
		...
		m.toDisplayName = function( map ){
			...
			var ret = this.firstValid([
						map.get('$nickname'),
						this.join([map.get('Prefix'),map.get('$firstname'),map.get('Middle name'),map.get('$lastname'),map.get('Suffix')],' '),
						map.get('$company'),
						map.get('$tel_mobile'),
						map.get('$tel_home'),
						map.get('$tel_work'),
	...
	getManager_ja : function(){
		...
		m.toDisplayName = function( map ){
			...
			var ret = this.firstValid([
						map.get('$nickname'),
						this.join([this.firstValid([map.get('$lastname'),map.get('姓（フリガナ）')]),this.firstValid([map.get('$firstname'),map.get('名（フリガナ）')])],''),
						map.get('$company'),
						map.get('$tel_mobile'),
						map.get('$tel_home'),
						map.get('$tel_work'),

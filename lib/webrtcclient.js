...
(function (WebrtcClient) {
  var WEBRTC_CLIENT_VERSION = '2.0.17.276',
    Phone,
    Logger,
    jssipHack,
    jssipRtcSessionHack,
  ...
  Phone.prototype = {
    ...
    sendDTMF: function (tones, sessionId, options) {
      ...
      if (int(this.dtmfSendMode) === 1) {
        sendInbandDTMF = function () {
          var duration, interToneGap, tasksLengthOrg;
    ...
    _tryVideoCall: function (sessionId) {
      ...
      doCallFunc = function (target) {
        var i, session, sourceSessionId, target;
        ...
        if (session.videoOptions && session.videoOptions.shareStream) {
          sourceSessionId =
            (self._ridVideoClientSessionsTable[rid] &&
              Object.keys(self._ridVideoClientSessionsTable[rid])[0]) ||
            null;
          if (
            sourceSessionId &&

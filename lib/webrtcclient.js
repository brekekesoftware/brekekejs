...
(function(WebrtcClient) {
  var WEBRTC_CLIENT_VERSION = '2.0.11.248',
    Phone,
    Logger,
    jssipHack,
    jssipRtcSessionHack,
  ...
  Phone.prototype = {
    ...
    startWebRTC: function(configuration) {
      ...
      url = string(configuration.url || defaultConfiguration.url);
      if (url) {
     ...
     */
    checkUserMedia: function(callback, options, screenCapture) {
    ...
    setWithVideo: function(sessionId, withVideo, videoOptions, exInfo) {
      ...
      if (
        session.rtcSession &&
        session.rtcSession.isEstablished &&
        session.rtcSession.isEstablished()
      ) {
    ...
    _doUaCall: function(
      ...
      // ice candidate gathering timeout timer
      if (
        this._sessionIceCandidateInfoTable['outgoing'] &&
        this._sessionIceCandidateInfoTable['outgoing'].gatheringTimer
      ) {
        clearTimeout(
          this._sessionIceCandidateInfoTable['outgoing'].gatheringTimer,
        );
    ...
    _tryVideoCall: function(sessionId) {
      ...
      if (mustUpdateRemoteUserOptions) {
        if (
          ...
          session.rtcSession.on(
            'confirmed',
            by(this, this._sendInfoXUaEx, [sessionId, true, null, 0]),
          );
        }
      }
    ...
    _ua_newRTCSession: function(e) {
      ...
      if (data.session.direction === 'outgoing') {
        ...
        if (this._sessionIceCandidateInfoTable['outgoing']) {
          this._sessionIceCandidateInfoTable[
            sessionId
          ] = this._sessionIceCandidateInfoTable['outgoing'];
          this._sessionIceCandidateInfoTable[
            sessionId
          ].onGatheringTimedOut = by(this, this._terminateRtcSession, [
            data.session,
          ]);
          delete this._sessionIceCandidateInfoTable['outgoing'];
        }
      } else {
